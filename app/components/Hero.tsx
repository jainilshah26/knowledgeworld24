...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const onEnter = () => {
      ...
      if (cur2) { cur2.style.width = '46px'; cur2.style.height = '46px'; cur2.style.borderColor = 'rgba(46,158,115,.7)' }
    }
    const leaveHandlers = new Map<HTMLElement, () => void>()
    hoverEls.forEach(el => {
      const onLeave = () => {
        if (cur) { cur.style.width = '7px'; cur.style.height = '7px' }
        if (cur2) { cur2.style.width = '30px'; cur2.style.height = '30px'; cur2.style.borderColor = 'rgba(46,158,115,.4)' }
        el.style.transform = ''
      }
    ...
    if (canvas && hero) {
      ...
      pointsMat = new THREE.PointsMaterial({ color: 0x2e9e73, size: 1.5, transparent: true, opacity: .9, blending: THREE.AdditiveBlending, depthWrite: false, fog: true })
      ...
      linesMat = new THREE.LineBasicMaterial({ color: 0x2e9e73, transparent: true, opacity: .16, blending: THREE.AdditiveBlending, depthWrite: false, fog: true })
      const lines = new THREE.LineSegments(lGeo, linesMat)
      scene.add(lines)

      icoGeo = new THREE.IcosahedronGeometry(24, 1)
      icoMat = new THREE.MeshBasicMaterial({ color: 0x2e9e73, wireframe: true, transparent: true, opacity: .1, fog: true })
  ...
  return (
    <>
      ...
      <style jsx global>{`
        #cur { position: fixed; width: 7px; height: 7px; background: var(--accent); border-radius: 50%; pointer-events: none; z-index: 9999; transform: translate(-50%,-50%); transition: width .25s, height .25s, opacity .25s; }
        #cur2 { position: fixed; width: 30px; height: 30px; border: 1px solid rgba(46,158,115,.4); border-radius: 50%; pointer-events: none; z-index: 9998; transform: translate(-50%,-50%); transition: left .12s ease-out, top .12s ease-out, width .25s, height .25s, border-color .25s; }
        ...
        .hero .badge { display: inline-flex; align-items: center; gap: 9px; border: 1px solid rgba(46,158,115,.3); background: rgba(11,11,13,.6); padding: 7px 16px; border-radius: 100px; font-size: 11px; letter-spacing: 1.2px; text-transform: uppercase; color: rgba(255,255,255,.5); margin-bottom: 30px; opacity: 0; animation: heroFade .8s ease .2s forwards; }
        .hero .badge i { width: 6px; height: 6px; background: var(--accent); border-radius: 50%; animation: heroBlink 1.4s infinite; }
        ...
        .hero .rule { width: 60px; height: 1px; background: linear-gradient(90deg,transparent,var(--accent),transparent); margin: 26px 0; opacity: 0; animation: heroFade .9s ease .75s forwards; }
        ...
        .hero .bp { display: inline-flex; align-items: center; gap: 9px; background: linear-gradient(135deg,#a8d9c4,var(--accent) 60%,#1f7355); color: #000; font-family: var(--display); font-size: 13px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; padding: 15px 30px; border-radius: 3px; text-decoration: none; transition: box-shadow .3s; will-change: transform; }
        .hero .bp:hover { box-shadow: 0 0 44px rgba(46,158,115,.5); }
        ...
        .hero .bs:hover { color: #fff; border-color: rgba(46,158,115,.5); }

...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const onEnter = () => {
      ...
      if (cur2) { cur2.style.width = '46px'; cur2.style.height = '46px'; cur2.style.borderColor = 'rgba(184,145,43,.7)' }
    }
    const leaveHandlers = new Map<HTMLElement, () => void>()
    hoverEls.forEach(el => {
      const onLeave = () => {
        if (cur) { cur.style.width = '7px'; cur.style.height = '7px' }
        if (cur2) { cur2.style.width = '30px'; cur2.style.height = '30px'; cur2.style.borderColor = 'rgba(184,145,43,.4)' }
        el.style.transform = ''
      }
    ...
    if (canvas && hero) {
      ...
      scene.fog = new THREE.FogExp2(0xf7f3ea, 0.0065)
      ...
      pointsMat = new THREE.PointsMaterial({ color: 0xb8912b, size: 1.5, transparent: true, opacity: .9, blending: THREE.AdditiveBlending, depthWrite: false, fog: true })
      ...
      linesMat = new THREE.LineBasicMaterial({ color: 0xb8912b, transparent: true, opacity: .16, blending: THREE.AdditiveBlending, depthWrite: false, fog: true })
      const lines = new THREE.LineSegments(lGeo, linesMat)
      scene.add(lines)

      icoGeo = new THREE.IcosahedronGeometry(24, 1)
      icoMat = new THREE.MeshBasicMaterial({ color: 0xb8912b, wireframe: true, transparent: true, opacity: .1, fog: true })
  ...
  return (
    <>
      ...
      <section className="hero" id="hero" ref={heroRef}>
        ...
        <div className="left">
          ...
          <div className="actions">
            <a href="/free-audit" className="bp" data-magnet>Get Free Audit
      ...
      <style jsx global>{`
        ...
        .hero .sub { font-size: 16.5px; font-weight: 300; color: rgba(26,23,18,.46); line-height: 1.75; max-width: 460px; margin: 0 auto 38px; opacity: 0; animation: heroFade .9s ease .85s forwards; }
        .hero .actions { display: flex; justify-content: center; gap: 13px; opacity: 0; animation: heroFade .9s ease 1s forwards; }
        .hero .bp { display: inline-flex; align-items: center; gap: 9px; background: linear-gradient(135deg,#e6c876,var(--accent) 60%,#8a6a1e); color: #1a1712; font-family: var(--display); font-size: 13px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; padding: 15px 30px; border-radius: 3px; text-decoration: none; transition: box-shadow .3s; will-change: transform; }
        .hero .bp:hover { box-shadow: 0 0 44px rgba(184,145,43,.5); }
        .hero .bs { display: inline-flex; align-items: center; color: rgba(26,23,18,.46); font-size: 13px; letter-spacing: .3px; padding: 15px 22px; border: 1px solid #e4dcc8; background: rgba(255,255,255,.5); border-radius: 3px; text-decoration: none; transition: all .3s; will-change: transform; }
        .hero .bs:hover { color: #1a1712; border-color: rgba(184,145,43,.5); }
